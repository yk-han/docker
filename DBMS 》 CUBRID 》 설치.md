...
압축 푸는 경로 지정 또는 압축을 푼 후 원하는 위치로 이동
 -> d:\Server_Program\Cubridmanager
압축 푼 후 실행 시 경로 지정
 -> d:\Server_Storage\Cubrid-10.2 
...
* SSH, FTP 접속
 - SSH 접속 작업
  1. 계정 변경
   -> su - cubrid
  2. 백업 경로 이동
   -> cd /home/cubrid/db_backup
  3. 언로드 백업 (비밀번호 확인)
   -> cubrid unloaddb keepdb@localhost 
      Cubrid12#$
 - FTP 접속 작업
  1. 경로 이동 
   -> /home/cubrid/db_backup
  2. 생성된 4개 파일
     로컬pc로 내려받은 후 외장하드로 복사, 이파일명 변경(파일명 예(220916)keepdb@~)
   -> keepdb@localhost_indexes
      keepdb@localhost_objects
      keepdb@localhost_schema
      keepdb@localhost_unloaddb.log
```
...
3. 생성된 db 마우스 우측 버튼 > 로그인, 데이터베이스 정지 상태로 만듦 > 데이터베이스 관리 > 데이터베이스 로드
   아래와 같이 매핑
 -> v 스키마 keepdb@localhost_schema
    v 데이터 keepdb@localhost_objects
	v 인덱스 keepdb@localhost_indexes
	  문법검사       ->원복 속도 향상
